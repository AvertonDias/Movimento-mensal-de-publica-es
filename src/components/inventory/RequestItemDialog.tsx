...
import { PackageSearch, CheckCircle2, Truck, PlusCircle, Hash, StickyNote, Trash2, History, CalendarDays, X, AlertTriangle } from 'lucide-react';
...
export function RequestItemDialog({ item, onClose, targetUserId }: RequestItemDialogProps) {
  ...
  const [deleteConfirmRequest, setDeleteConfirmRequest] = useState<ItemRequest | null>(null);
  ...
  const confirmDeleteRequest = () => {
    ...
    if (!activeUid || !db || !item || !req) return;
                      ...
                      <div key={req.id} className="bg-amber-50/50 border border-amber-100 rounded-xl p-4 space-y-3 shadow-sm">
                        <div className="flex justify-between items-start">
                          ...
                          <div className="flex gap-1">
                            ...
                            {!isHelperMode && (
                              <Button variant="ghost" size="icon" onClick={() => setDeleteConfirmRequest(req)} className="h-8 w-8 text-neutral-400 hover:text-destructive">
                  ...
                  <div className="bg-neutral-50 rounded-xl overflow-hidden border border-neutral-100">
                    {receivedRequests.map((req, idx) => (
                      <div key={req.id} className={cn("p-3 flex justify-between items-center", idx !== receivedRequests.length - 1 && "border-b border-neutral-100")}>
                        ...
                        {!isHelperMode && (
                          <Button variant="ghost" size="icon" className="h-7 w-7 text-neutral-300 hover:text-destructive" onClick={() => setDeleteConfirmRequest(req)}>
                            <Trash2 className="h-3.5 w-3.5" />
                          </Button>
                        )}
                      </div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-left font-bold uppercase text-xs leading-relaxed">
              Deseja realmente remover este registro de pedido? Esta ação não pode ser desfeita.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
