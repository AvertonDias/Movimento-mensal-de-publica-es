      ...
      <Table className="border-collapse table-fixed w-[750px] print:w-[750px]">
        <TableHeader className="print:table-row-group">
          <TableRow className="border-b border-black bg-white hover:bg-white h-[22px]">
            <TableHead colSpan={2} className="w-[180px] text-[8px] font-black uppercase text-black p-0 h-auto text-center border-r border-black leading-none">MÊS E ANO</TableHead>
            {lastSixMonths.map((month, mIdx) => (
              <TableHead 
                key={month.key} 
                colSpan={3} 
                className={cn(
                  "text-center text-[8px] font-black uppercase text-black p-0 h-auto border-r border-black bg-neutral-50/50 leading-none",
          ...
          <TableRow className="border-b border-black bg-white hover:bg-white h-[22px]">
            <TableHead className="w-[25px] text-[7px] font-bold text-black p-0 h-auto text-center leading-none border-r border-black">N.º</TableHead>
            <TableHead className="w-[155px] text-[10px] font-black text-black px-1 py-0 h-auto align-middle leading-none border-r border-black">Publicações</TableHead>
            
            {lastSixMonths.map((m, mIdx) => (
              <React.Fragment key={m.key}>
                <TableHead className="w-[31px] text-[5.5px] font-bold text-black p-0 h-auto text-center uppercase tracking-tighter leading-none border-r border-black">Rec.</TableHead>
                <TableHead className="w-[32px] text-[5.5px] font-bold text-black p-0 h-auto text-center uppercase tracking-tighter leading-none border-r border-black">Est.</TableHead>
                <TableHead className={cn(
                  "w-[32px] text-[6px] font-black text-black p-0 h-auto text-center uppercase bg-neutral-200 leading-none border-r border-black",
                  mIdx === lastSixMonths.length - 1 && "border-r-0"
                )}>Saí.</TableHead>
              </React.Fragment>
            ))}
          ...
          {combinedItems.map((item, idx) => {
            ...
            return (
              <TableRow key={`hist-row-${itemId}-${idx}`} className="border-b border-black h-5 hover:bg-transparent print:h-4">
                ...
                <TableCell className="w-[155px] text-[9px] px-1 py-0 border-r border-black h-full overflow-hidden leading-none">
                ...
                {lastSixMonths.map((m, mIdx) => (
                  <React.Fragment key={m.key}>
                    ...
                    <TableCell className={cn(
                      "text-[8px] text-center p-0 font-black bg-neutral-200 leading-none border-r border-black",
                      mIdx === lastSixMonths.length - 1 && "border-r-0"
